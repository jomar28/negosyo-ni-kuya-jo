...
function EditTsikotModal({
  ...
  // ... (Keep all your existing handlers: handleChange, handleAttemptCancel, handleAddMisc, etc. UNCHANGED)
  ...
  function handleUpdateMisc() {
    ...
    const updatedMiscList = editForm.miscellaneous.map((item, index) => {
      if (index === editingMiscIndex) { return updatedExpense; }
      return item;
    });
    setEditForm(prev => ({ ...prev, miscellaneous: updatedMiscList }));
    setNewMisc({ date: formatDate(new Date()), amount: '', notes: '' });
  ...
  async function handleSave() {
    ...
    const { error } = await supabase.from('tsikot').update({
        car: editForm.car,
    ...
    <div className='fixed inset-0 z-[100] bg-stone-900/75 backdrop-blur-sm flex items-center justify-center p-4'>
      ...
      >
        <div className='p-6 overflow-y-auto'>
          ...
          {/* ... (Your existing form grid - Car, Date, Price, Misc - UNCHANGED) ... */}
          <div className='grid grid-cols-10 gap-y-3 gap-x-2 mb-6 border-b-2 border-black pb-4'>
            <div className='flex flex-col col-span-10'>
              ...
              <input type='text' name='car' placeholder='Car Model' className={inputStyle} value={editForm.car} onChange={handleChange} />
            </div>
            <div className='flex flex-col col-span-4 md:col-span-5'>
              ...
              <input type='date' name='date_bought' className={inputStyle} value={editForm.date_bought} onChange={handleChange} />
            </div>
            <div className='flex flex-col col-span-4 col-start-6 justify-end md:col-span-5 md:col-span-5 md:col-start-1 md:row-start-3'>
              <label className='text-sm font-bold mb-1 text-stone-700'>Date Sold (opt)</label>
              <input type='date' name='date_sold' className={inputStyle} value={editForm.date_sold} onChange={handleChange} />
            </div>
            <div className='flex flex-col col-span-5 md:col-span-5 md:col-start-6 md:row-start-2'>
              ...
              <input type='number' step='0.01' name='buy_price' placeholder='Buy Price' className={`${inputStyle} [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none`} value={editForm.buy_price} onChange={handleChange} />
            </div>
            <div className='flex flex-col col-span-5 md:col-span-5 md:col-start-6 md:row-start-3'>
              ...
              <input type='number' step='0.01' name='sell_price' placeholder='Sell Price' className={`${inputStyle} [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none`} value={editForm.sell_price} onChange={handleChange} />
            </div>
          </div>
          ...
          <div className='space-y-2 mb-4 max-h-40 overflow-y-auto pr-2'>
            {!Array.isArray(editForm.miscellaneous) || editForm.miscellaneous.length === 0 ? (
              ...
              editForm.miscellaneous.map((item, index) => (
                <div key={index} className={`flex items-center justify-between p-2 border-2 border-black cursor-pointer transition-colors bg-[#F0EFEA] ${editingMiscIndex === index ? 'bg-indigo-200' : 'hover:bg-stone-400'}`} onClick={() => handleStartEditMisc(index)}>
                  <div className='flex-1 flex items-center text-sm overflow-hidden'>
                    ...
                    <span className='px-2 text-stone-400'>|</span>
                    <span className='flex-1 text-stone-600 truncate' title={item.notes || 'No Notes'}>{item.notes || 'No Notes'}</span>
                  </div>
                  <button type='button' onClick={(e) => { e.stopPropagation(); handleRemoveMisc(index); }} className='text-rose-600 hover:text-rose-800 text-xs ml-2 font-bold uppercase'>Remove</button>
                </div>
              ))
            )}
          </div>
          ...
          <div className='pt-3 mt-3 border-t-2 border-black'>
            <div className='grid grid-cols-10 gap-y-3 gap-x-2 items-end'>
              <div className='flex flex-col col-span-5 md:col-span-3'>
                ...
                <input type='date' className={inputStyle} value={newMisc.date} onChange={e => setNewMisc({ ...newMisc, date: e.target.value })} />
              </div>
              <div className='flex flex-col col-span-10 md:col-span-2'>
                ...
                <input type='number' step='0.01' placeholder='0.00' className={`${inputStyle} [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none`} value={newMisc.amount} onChange={e => setNewMisc({ ...newMisc, amount: e.target.value })} />
              </div>
              <div className='flex flex-col col-span-10 md:col-span-3'>
                ...
                <input type='text' placeholder='Expense details...' className={inputStyle} value={newMisc.notes} onChange={e => setNewMisc({ ...newMisc, notes: e.target.value })} />
              </div>
              <div className='col-span-10 md:col-span-2'>
                <div className='flex gap-2 w-full md:w-auto'>
                  <button type='button' onClick={editingMiscIndex !== null ? handleUpdateMisc : handleAddMisc} className={`flex-1 md:flex-none md:w-[38px] h-[38px] text-white flex items-center justify-center transition-colors border-2 border-black rounded-none ${editingMiscIndex !== null ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-green-600 hover:bg-green-700'}`} title={editingMiscIndex !== null ? "Update Expense" : "Add Expense"}>
                    {editingMiscIndex !== null ? <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg> : <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
                  </button>
                  {editingMiscIndex !== null && (
                    <button type='button' onClick={() => { setNewMisc({ date: formatDate(new Date()), amount: '', notes: '' }); setEditingMiscIndex(null); }} className='flex-1 md:flex-none md:w-[38px] h-[38px] bg-stone-200 text-stone-600 hover:bg-stone-300 hover:text-rose-600 flex items-center justify-center transition-colors border-2 border-black rounded-none' title="Cancel Editing">
